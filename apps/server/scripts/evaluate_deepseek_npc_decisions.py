...
from trpg_server.characters.decision import DeepSeekNpcDecisionAdapter, SafeNpcDecider
...
DEFAULT_SUITE = (
    REPOSITORY_ROOT / "evals" / "npc-decision" / "gray-harbor-v1.json"
)
...
def main() -> int:
    ...
    suite = load_npc_decision_evaluation_suite(arguments.suite)
    decider = SafeNpcDecider(
        DeepSeekNpcDecisionAdapter(settings),
    ...
    if arguments.compact:
        ...
        report["comparisons"] = [
            result for result in report["comparisons"] if not result["passed"]
        ]
