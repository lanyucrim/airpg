...
不调用 AI。确实缺失时，一次模型请求同时给出规范名称、材质、稳定结构、粗略尺寸、
可观察特征、未知事实、日常类别、堆叠、明确计量单位、美元估价和克重，程序生成稳定
定义 ID 并按苹果基准计算克朗。生成期的结构化物理字段必须逐项出现在最终
`description` 中；材质无法确认时必须明确保留未知，不能由模型自行补满。
新说法经一次归一化发现属于正式图册定义时，只缓存该说法到正式 `definitionId` 的映射，
不复制正式记录；后续相同说法也不再调用 AI。

这些结构化物理字段只属于 AI 候选校验，不会增加物品基础字段。目录中的每个 `item`
仍必须通过当前正式 15 字段物品定义校验。目录根部记录物品契约版本、
